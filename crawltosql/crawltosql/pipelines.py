...
class CrawltosqlPipeline(object):
    def __init__(self):
        self.conn = pymysql.connect(host="localhost", user="root", passwd="ddd123", db="mydata")

    def process_item(self, item, spider):
        mylen = min(len(item["url"]), len(item["title"]), len(item["author"]), len(item["reply"]), len(item["scan"]),
                    len(item["pubtime"]), len(item["lastreplytime"]), len(item["endauthor"]))
